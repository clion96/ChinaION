...
export default function NavBar() {
    ...
    const [bgColor, setBgColor] = useState('bg-transparent'); // 添加了背景颜色的状态
    const location = useLocation();
    const Title = new URLSearchParams(location.search).get('Title');
    const [activeTab, setActiveTab] = useState(Title || 'homepage');
    ...
    useEffect(() => {
      const handleScroll = () => {
        const sixThirdsPoint = (window.innerHeight * 2) / 3; // 将计算调整为视口高度的 2/3
        ...
        // 根据滚动位置设置背景颜色
        if (window.scrollY < (window.innerHeight * 5) / 3) {
    ...
    return (
      <header className={`fixed w-full top-0 z-50 transition-transform transform duration-1000 ease-in-out ${
        isNavBarVisible ? 'translate-y-0 ' + bgColor : '-translate-y-full shadow' + bgColor
      }`}
    >
        <nav className={`w-full ${ navbar ? "bg-black" : "" }`}>
